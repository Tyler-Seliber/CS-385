...
//    compute the square as the shortest distance also has the shortest squeare of the distance -> faster to run
}

// pair comes from #include utility
// generic type that can have two different types inside
pair <Neighbor, Neighbor> nearest_neighbors(Neighbor neighbors[], const int length) {
    float min_distance = numeric_limits<float>::max(); // set to essentially +inf - maximum possible number of type float you can exist on your computer (depends on CPU)
    int min_i, min_j;
    ...
    for (int i = 0; i < length - 1; i++) {
        for (int j = i + 1; j < length; j++) { // don't start at 0 because you would look at the same pair twice
...
int main() {
    ...
    // result.first = first element in pair, result.second = second element in pair
